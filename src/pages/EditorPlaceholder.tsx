...
// EXPERIMENTAL (experimental/develop): handleRecommend no longer produces
// this sentinel (it always calls the real backend now — see handleRecommend
// below), but handleFeedback still branches on it for any layout a user
// already has saved locally from before this change, so a stale session
// doesn't hard-crash trying to hit the network with an invalid layoutId.
...
export default function EditorPlaceholder() {
  ...
  const handleRecommend = async () => {
    ...
    // EXPERIMENTAL (experimental/develop): the scripted-mood shortcuts
    // (isHobbyCoralRecommendationSelected / currentScenario) used to take
    // over here instead of calling the real backend, for exactly the
    // purpose/style/palette combos a judge/tester is most likely to try
    // first ("휴식+우드+내추럴", "업무+모던+그레이", "hobby"+"pink"). That's
    // why colorTone/lifestyle changes looked like they weren't reflected —
    // the real (already-fixed) backend was never being called for those
    // combos. Routing everything through the real backend call below
    // instead, so what a tester sees always reflects the actual
    // recommendation service. isCollectorRoom's two named demo rooms are
    // unaffected either way — they already always went to the backend,
    // which has its own separate scripted path for those two exact rooms.
    ...
    try {
      // Real backend round trip. A local backend answers near-instantly,
      // which reads as an instant swap instead of the AI actually working —
      // race it against a fixed 5s floor so "AI 추천 생성 중..." always shows
      // for a consistent minimum duration.
